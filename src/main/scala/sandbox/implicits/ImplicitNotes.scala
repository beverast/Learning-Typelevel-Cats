...
import sandbox.implicits.{Json, JsonWriter, Person, Cat, Printable}
...
object ImplicitNotes {
  ...
  /*
   ...
   - Covariance:      trait List[+A]  =>   F[B] is a subtype of F[A] if B is a subtype of A
   - Contravariance:  trait F[-A]     =>   F[B] is a subtype of F[A] if A is a subtype of B
   - Invariance:      trait F[A]      =>   F[A] and F[B] are never subtypes of one another
   */
}
